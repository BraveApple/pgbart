...
Bart::Bart(const Data& data_train, const Param& param, const Control& control, const Cache& cache, const CacheTemp& cache_temp, Random& pgrandom)
  : pred_val_mat_train(data_train.n_point, control.m_bart) {
  this->p_particles.clear();
  this->pmcmc_objects.clear();

  this->p_treemcmcs.clear();

  this->update_pred_val_sum();
  for (UINT i = 0; i < control.m_bart; i++) {
	  if (control.mcmc_type == "pg") {
		  Pmcmc_Ptr pmcmc_ptr = init_particle_mcmc(data_train, control, param, cache, cache_temp, pgrandom);
		  Particle_Ptr p_ptr = pmcmc_ptr->p_ptr;
		  sample_param(p_ptr, param, false, pgrandom);
		  ...
		  sample_param(treemcmc_ptr, param, false, pgrandom);
...
void Bart::update_pred_val(UINT i_t, const Data& data_train, const Param& param, const Control& control) {
	IntVector_Ptr leaf_id_ptr;
	DoubleVector_Ptr temp_ptr;
	if (control.mcmc_type == "pg") {
...
void Bart::sample_labels(Data& data, const Param& param, Random& pgrandom) {
  data.y_original = pred_val_sum_train + pgrandom.simulate_normal_distribution(0.0, 1.0, data.n_point) / std::sqrt(param.lambda_bart);
}

void Bart::sample_lambda_bart(const Data& data, Param& param, Random& pgrandom) {
  double lambda_alpha = param.alpha_bart + 0.5 * data.n_point;
  double lambda_beta = param.beta_bart + 0.5 * sum2(data.y_original - this->pred_val_sum_train);
  param.lambda_bart = pgrandom.simulate_gamma_distribution(lambda_alpha, 1.0 / lambda_beta);
...
tuple<double, double> Bart::compute_train_loglik(const Data& data, const Param& param) {
  ...
  double loglik_train = 0.5 * data.n_point * (std::log(param.lambda_bart)
    - std::log(2 * PI) - param.lambda_bart * mse_train);
...
void sample_param(Particle_Ptr p_ptr, const Param& param, bool set_to_mean, Random& pgrandom) {
  ...
  */
  math::initial_map(p_ptr->tree_ptr->pred_val_n, p_ptr->tree_ptr->leaf_node_ids, DBL_MAX);
  ...
  for (auto node_id : p_ptr->tree_ptr->leaf_node_ids) {
    ...
    if (set_to_mean)
      ...
      p_ptr->tree_ptr->pred_val_n[node_id] = pgrandom.simulate_normal_distribution(0.0, 1.0) / std::sqrt(mu_prec_post) + mu_mean_post;
    }
    p_ptr->pred_val_logprior += compute_normal_loglik(p_ptr->tree_ptr->pred_val_n[node_id], param.mu_mean, param.mu_prec);
  }
}

void sample_param(TreeMCMC_Ptr t_ptr, const Param& param, bool set_to_mean, Random& pgrandom) {
    ...
    std::cerr << node_id << " " << mu_prec_post << " " << mu_mean_post << "\n";

		if (set_to_mean)
			t_ptr->tree_ptr->pred_val_n[node_id] = 0.0 + mu_mean_post;
		else {
			t_ptr->tree_ptr->pred_val_n[node_id] = pgrandom.simulate_normal_distribution(0.0, 1.0) / std::sqrt(mu_prec_post) + mu_mean_post;
		}
