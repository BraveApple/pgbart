...
Pmcmc::Pmcmc(const Data& data, const Control& control, const Param& param, const Cache& cache, const CacheTemp& cache_temp, Random& pgrandom) {
  ...
  this->p_ptr = nullptr;
  this->update_p(particles_ptr, log_weights_ptr, log_pd, control, pgrandom);
}

bool Pmcmc::update_p(Vec_Particle_Ptr particles_ptr, DoubleVector_Ptr log_weights_ptr, double& log_pd, const Control& control, Random& pgrandom) {
  ...
  UINT k = pgrandom.sample_multinomial_distribution(softmax(*log_weights_ptr));
...
bool Pmcmc::sample(const Data& data, const Control& control, const Param& param, const Cache& cache, const CacheTemp& cache_tmp, Random& pgrandom) {
  ...
  DoubleVector_Ptr log_weights_ptr;
  double log_pd;
  std::tie(particles_ptr, log_pd, log_weights_ptr) = init_run_smc(data, control, param, cache, cache_tmp, this->p_ptr, pgrandom);
  // update the tree in the mcmc_object
  bool change = update_p(particles_ptr, log_weights_ptr, log_pd, control, pgrandom);
  return change;
}
...
Pmcmc_Ptr init_particle_mcmc(const Data& data_train, const Control& control, const Param& param,
  ...
  Pmcmc_Ptr pmcmc_ptr = make_shared<Pmcmc>(data_train, control, param, cache, cache_temp, pgrandom);
  return pmcmc_ptr;
}
...
tuple<Particle_Ptr, bool> run_particle_mcmc_single_tree(const Control& control,
  ...
  // starting mcmc
  change = pmcmc_ptr->sample(data_train, control, param, cache, cache_temp, pgrandom);
...
IntVector_Ptr TreeMCMC::get_nodes_subtree(const int node_id) {
  ...
  std::queue<int> expand;
  expand.push(node_id);
  while (expand.size() > 0) {
    const int node = expand.front();
    expand.pop();
...
double TreeMCMC::compute_log_inv_acc_p(const int node_id, const Param& param, const int len_both_children_terminal,
  ...
  if (!compare::compare_if_zero(logprior_children - this->logprior[left] - this->logprior[right])) {
	...
	std::cout << "trem1 = " << logprior_children << "\n";
	std::cout << "trem2 = " << this->logprior[left] + this->logprior[right] << "\n";
	std::cout << "trem2a = " << this->logprior[left] << "\n";
	std::cout << "trem2b = " << this->logprior[right] << "\n";
	std::cout << "node_id = " << node_id << "\n";
	std::cout << "left = " << left << "\n";
	std::cout << "right = " << right << "\n";
	//std::cout << "logprior = " << right << "\n";
...
bool TreeMCMC::grow(const Data& train_data, const Control& control, const Param& param, const Cache& cache,
  ...
  if (grow_nodes.size() == 0)
    return change;
  const int node_id = pgrandom.ramdom_choice(grow_nodes);
  ...
  tie(do_not_split_node_id, split_info_ptr, logprior_nodeid) =
    this->sample_split_prior(node_id, train_data, param, control, cache, pgrandom);
  ...
  const double log_r = std::log(pgrandom.simulate_continuous_uniform_distribution(0, 1));
...
bool TreeMCMC::prune(const Data& train_data, const Control& control, const Param& param, const Cache& cache,
  ...
  const int node_id = pgrandom.ramdom_choice(this->both_children_terminal);
  const int left = this->tree_ptr->getLeftNodeID(node_id);
  const int right = this->tree_ptr->getRightNodeID(node_id);
...
bool TreeMCMC::change(const Data& train_data, const Control& control, const Param& param, const Cache& cache,
  ...
  const int node_id = pgrandom.ramdom_choice(this->tree_ptr->non_leaf_node_ids);
  bool do_not_split_node_id; SplitInfo_Ptr split_info_ptr; double logprior_nodeid;
  tie(do_not_split_node_id, split_info_ptr, logprior_nodeid) =
    this->sample_split_prior(node_id, train_data, param, control, cache, pgrandom);
...
bool TreeMCMC::swap(const Data& train_data, const Control& control, const Param& param, const Cache& cache,
  ...
  const int id = pgrandom.simulate_discrete_uniform_distribution(0, this->inner_pc_pairs.size() - 1);
  const int node_id = this->inner_pc_pairs[id].parent;
  const int child_id = this->inner_pc_pairs[id].child;

  UINT length = this->inner_pc_pairs.size();
  IntVector_Ptr nodes_subtree_ptr = this->get_nodes_subtree(node_id);
...
bool TreeMCMC::sample(const Data& train_data, const Control& control, const Param& param,
  const Cache& cache, Random& pgrandom) {

  const int move_type = pgrandom.simulate_discrete_uniform_distribution(0, 3);
  ...
  switch(move_type) {
  case 0: change = this->grow(train_data, control, param, cache, grow_nodes, pgrandom);
	  break;
  case 1: change = this->prune(train_data, control, param, cache, grow_nodes, pgrandom);
	  break;
  case 2: change = this->change(train_data, control, param, cache, grow_nodes, pgrandom);
	  break;
  case 3: change = this->swap(train_data, control, param, cache, grow_nodes, pgrandom);
	  break;
...
tuple<TreeMCMC_Ptr, bool> run_cgm_mcmc_single_tree(TreeMCMC_Ptr tree_mcmc_ptr, const Control& control,
  ...
  const bool change = tree_mcmc_ptr->sample(train_data, control, param, cache, pgrandom);
